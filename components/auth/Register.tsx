...
import fetchCountries from "@/constants/fetchCountries";
...
const Register = (props: RegisterProps) => {
  ...
  useEffect(() => {
    const fetData = async () => {
      try {
        const allCountry = await fetchCountries();
        const displayData = allCountry.map(
          (country: { name: { common: any } }) => {
            const countryName = country.name.common;
            return { label: countryName, value: countryName };
          }
        );
        setDisplyCountry(displayData);
      } catch (error: any) {
  ...
  const registerHandler = async () => {
    ...
    if (validator.isEmpty(inputs.country.trim())) {
